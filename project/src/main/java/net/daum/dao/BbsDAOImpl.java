...
public class BbsDAOImpl implements BbsDAO {
	...
	public BbsVO getBbsCont(int bbs_no) {
		...
		/*getReferenceById() JPA 수행 메서드 특징
		 * null이 발생하지 않는 경우 예외오류가 발생한다. null이 발생하지 않는 경우에 사용한다*/
		return bc;
	}//내용보기

	@Override
	public void updateLevel(BbsVO b) {
		//this.sqlSession.update("levelUp", b);//mybatis에서는 update()메서드로 레코드를 수정
		//levelUp은 bbs.xml에 설정할 유일한 아이디명
	...
	public void replyBbs(BbsVO b) {
		...
		System.out.println(" \n ==================>JPA로 답변 저장");
		int bbs_no=this.sqlSession.selectOne("bbsNoSeq_Find");//시퀀스로 부터 번호값을 구함
		b.setBbs_no(bbs_no);;//자료실 번호값 저장
	...
	public void delBbs(int bbs_no) {
		//this.sqlSession.delete("bbs_del", bbs_no);//mybatis에서 delete()메서드는 레코드를
		//삭제한다.
		
		System.out.println(" \n =================>JPA로 자료실 삭제");
		this.bbsRepo.deleteById(bbs_no);//번호를 기준으로 삭제
	}//자료실 삭제
}
