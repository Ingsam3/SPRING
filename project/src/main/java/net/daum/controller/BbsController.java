...
public class BbsController {
	...
	public String bbs_reply_ok(BbsVO b,int page) {
		...
		/*BbsVO b라고 하면 bbs_reply.jsp의 네임 파라미터 이름과 
		 * BbsVO의 변수명이이 일치하면 b 객체의 bbs_reply.jsp 에서 넘어온 값이 저장되어있다
		하지만 page는 변수로 정의 안 되어 있어 b객체의 값이 없다 따라서 int page로 별도로 가져와야한다
		*/
		this.bbsService.replyBbs(b); //답변 레벨업과 답변 저장
		return "redirect:/bbs_list?page="+page; //목록보기로 이동
	}//bbs_reply_ok()
	...
	//자료실 삭제
	@RequestMapping("/bbs_del_ok") //get & post로 전달되는 매핑 주소를 처리
	...
	throws Exception{
		...
		if(!db_pwd.getBbs_pwd().equals(del_pwd)) {
			...
			if(db_pwd.getBbs_file() != null) { //기존 첨부 파일이 있는 경우
				File delFile = new File(delFolder+db_pwd.getBbs_file());
				//삭제할 파일 객체 생성
				delFile.delete();//폴더는 삭제 안 됨, 기존 파일만 삭제 됨 
