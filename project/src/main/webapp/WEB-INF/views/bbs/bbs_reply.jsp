...
<body>
 <div id="bsW_wrap">
   ...
   <form method="post" action="bbs_reply_ok" onsubmit="return write_check();">
    ...
    <%-- 원본글이면 0, 첫번째 답변글이면 1,두번째 답변글이면 2.. 즉 원본글과 답변글을 구분하는 번호이면서
    몇번째 답변글인가를 알려준다.--%>
    <input type="hidden" name="bbs_level" value="${b.bbs_level}" >
    <%--답변글 정렬순서 --%>
    
    <%--페이징 즉 쪽나누기에서 내가본 쪽번호로 바로 이동하기 위한 책갈피 기능때문에 히든으로 쪽번호를 전달 --%>
    ...
    <table id="bsW_t">
     <tr>
      ...
      <td><input name="bbs_name" id="bbs_name" size="14" ></td>
         <%-- input 태그에서 type속성 생략하면 기본값이 text이다. --%>
     </tr>
     <tr>
      <th>글제목</th>
     ...
     <tr>
      <th>글내용</th>
      <td><textarea name="bbs_cont" id="bbs_cont" rows="8" cols="34"></textarea>
     </tr>     
    </table>
    <div id="bsW_menu">
     <input type="submit" value="답변" > <input type="button" value="취소"
