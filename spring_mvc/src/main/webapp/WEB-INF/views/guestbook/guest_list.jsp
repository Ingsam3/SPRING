...
<table border="1">
 ...
 <c:if test="${!empty glist}"> <%--목록이 있는 경우 실행 --%>
  ...
  <tr>
   <th>${g.gno}</th>
   <th>
    <a href="/controller/guest/guest_cont?gno=${g.gno}&page=${page}">${g.gtitle}</a>
    <%-- guest_cont?gno=번호값&page=쪽번호 get방식으로 gno에 번호,page에 쪽번호 2개의 피라미터 값이 전달됨.
 ...
 <tr>
  <td colspan="5" align="right">
   <input type="button" value="글쓰기" onclick=
	   "location='/controller/guest/guest_write?page=${page}';" >
	   <%--자바스크립트 location객체에 의해서 이동하는 것은 get방식이다. 페이징에서 내가 본 쪽번호로 바로 이동하는
	   책갈피 기능을 구현하기 위해서 guest_write?page=쪽번호 get방식으로 전달한다.--%>
  </td>
 </tr>
</table>
